...
DcWorld                              // rewrite of SimWorld, same tick-driver contract
  ...
  float FeedCapKw, FeedLoadKw, BandwidthCap, BandwidthUsed
...
Building
  ...
  BuildingState State  // Booting, Online, Warm25, Hot50, HeatShutdown,
                       // TrippedDark, NoUplink, Starving, ToggledOff
  float BootRemaining, ServedPf, TileTemp; bool ToggledOff; long PlacedTick
...
ContractScript                       // hardcoded, no generator
  ...
  void Step(DcWorld)                 // trigger offers, check completion (gpuOnline>=needs),
                                     //   deadline → penalty + toast; PASS → re-offer next day
...
HudController   // top bar: Cash | NET | Served/Cap PF + queue + breakeven ▲ | kW bar |
                ...
                // bottom: 4 build chips + locked silhouettes; overlay toggles HEAT/POWER/NET
...
GameController  // loop unchanged; speeds {0,1,3}; no cards; audio: 3 clips via
                //   AudioClip.Create (procedural sine/noise blips: money, trip, chime),
...
- Custom template (or `dist/index.html` post-step): `document.addEventListener('contextmenu', e => e.preventDefault())`; in bootstrap C#: `WebGLInput.captureAllKeyboardInput = false`.
- Known local gotchas (already documented in project memory): Brotli + `decompressionFallback` **not** needed here — this project builds with compression disabled and lets Cloudflare edge compress; keep `BuildScript.BuildWebGL` as-is. **Verify the ESM shield exists before first build:** `C:\Users\ericr\Unity\package.json` must contain `{"type":"commonjs"}` (stale `C:\Users\ericr\package.json` hijacks Unity's Node 12).
...
```
PicoChat: trigger=GpuUnlocked, needs 1 GPU online, advance 2500, adds 4 purple PF,
          rate +25% on its pool, deadline Day 3, penalty 1000
Nimbus:   trigger=Day 4, needs 4 GPU online, advance 8000, adds 20 purple PF,
          rate +50% on its pool, deadline Day 7, penalty 5000
Chips 1–8 exactly per DESIGN2 §5.4 (rewards 0/250/250/unlock/—/1500/500/open).
"Earned" = revenue + chip rewards + contract payments (NOT starting cash) — label the bar.
```
...
- Map 24×16 world units; camera orthographic, `orthographicSize ≈ 9.2`, positioned so the map occupies the right ~75% of the frame (x-center ≈ `GridW/2 − 3.5`), leaving a ~320px left UI gutter at 1280 wide. CanvasScaler stays 1600×900 reference.
...
5. **Purchases + amber ghost (1h):** BuyUplink/OrderSubstation + ETA; CheckPlace amber verdicts with reason strings. Gate: place GPU #2 → amber "PDU at 108%"; bw 11/10 → newest rack NO UPLINK; buy uplink → it feeds. **→ SMOKE WEBGL BUILD + deploy now (0.75h).**
...
8. **Aliveness (1.5h):** SpriteFactory additions, LED flipbook, fans, job dots + queue pile, cables, arc-flash, smoke, floaters kept, 3 procedural sounds. Gate: 30-second watch test — can you tell fed/starving/hot/dark racks apart with HUD hidden?
